...
from load_model import load_model_pretrained, oned_to_twod_bert_input, choose_model


class Twod_attn_map(nn.Module):
...
def seq_to_rna_index_onehot(seq):
    '''
    input:
...
def pair_attn_to_seq(pair_attn, data_seq):
    '''
    input:
    ...
    post_pair_attn = postprocess(pair_attn, data_seq, 0.01, 0.1, 100, 1.6, True,1.5)
    map_no_train = (post_pair_attn > 0.5).float().squeeze().cpu().numpy()
    pretrain_ss = stru_to_seq(map_no_train)
    return pretrain_ss
...
def predict(rna_lst, best_model_path=None, mlm_pretrained_model_path=None, arg_overrides=None, device='cpu'):
    ...
    # load model
    model_pre = load_model_pretrained(mlm_pretrained_model_path,arg_overrides)
    my_model = choose_model(model_pre.encoder)
    ...
    pretrain_model = Twod_attn_map(model_pre.encoder).to(device)
    pretrain_model.eval()
    ...
    pred_ss_lst = []
    for seq in rna_lst:
        X, data_seq = seq_to_rna_index_onehot(seq)
        one_d, twod_data = oned_to_twod_bert_input(X, len(seq))
        ...
        pair_attn = pred_ss
        fine_tune_ss = pair_attn_to_seq(pair_attn, data_seq)
        ...
        pretrain_ss = pair_attn_to_seq(pair_attn, data_seq)
        del pair_attn, pred_ss
...
if __name__ == '__main__':
    ...
    rna = ['GGGCAAUCCUCUUCGGGCCC','GGGCUAUUAGCUCAGUUGGUUAGAGCGCACCCCUGAUAAGGGUGAGGUCGCUGAUUCGAAUUCAGCAUAGCCCA']
    best_model_path = current_path + '/model/ss-rna_bpRNA_checkpoint_dp8/checkpoint_best.pt'
    mlm_pretrained_model_path = current_path + '/model/two_mlm_checkpoint/checkpoint_best.pt'
    arg_overrides = { "data": current_path + '/pretrain_dict/' }
    device='cpu'
    
    # Predicting the secondary structure of RNA
    pres_ss = predict(rna, best_model_path, mlm_pretrained_model_path, arg_overrides, device)
